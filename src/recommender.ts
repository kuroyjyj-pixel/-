import { SPOTS, RESTAURANTS, Spot, Restaurant } from "./data";
...
function scoreSpot(spot: Spot, req: PlanRequest): number {
  let score = 0;

  // Area match (high priority)
  const areaMatch = req.area.some((a) => spot.areaTag.includes(a));
  if (areaMatch) score += 40;
  else return -100; // Exclude spots outside requested areas

  // Style match
  req.style.forEach((s) => {
    if (spot.styleTag.includes(s)) score += 20;
  });

  // Family match
  if (
    spot.familyTag.includes("全員") ||
  ...
  req.likes.forEach((like) => {
    if (spot.likeTag.includes(like)) score += 10;
  });

  // Dislike penalty
  req.dislikes.forEach((dislike) => {
    if (spot.dislikeAvoid.includes(dislike)) score -= 30;
  });

  return score;
}

function scoreRestaurant(r: Restaurant, req: PlanRequest): number {
  let score = 0;
  ...
  // Style match
  req.style.forEach((s) => {
    if (r.styleTag.includes(s)) score += 15;
  });
  ...
  req.dislikes.forEach((d) => {
    if (r.dislikeAvoid.includes(d)) score -= 40;
  });
...
function generateTitle(req: PlanRequest): string {
...
function generateAdvice(req: PlanRequest): string {
...
const TIME_SLOTS = [
  { label: "午前 9:00〜11:30", category: "morning" as const },
  { label: "昼前 11:30〜13:00", category: "morning" as const },
  { label: "午後 13:00〜15:30", category: "afternoon" as const },
  { label: "夕方 15:30〜17:30", category: "afternoon" as const },
  { label: "夜 18:00〜20:00", category: "evening" as const },
];

export function buildPlan(req: PlanRequest): Plan {
  // Score and pick top spots
  const scoredSpots = SPOTS.map((s) => ({ spot: s, score: scoreSpot(s, req) }))
    .filter(({ score }) => score > 0)
    .sort((a, b) => b.score - a.score);
  ...
  const schedule: ScheduleItem[] = chosen.map((spot, i) => ({
  ...
  // Pick restaurants
  const lunchCandidates = RESTAURANTS.filter((r) => r.type === "昼食" || r.type === "ランチ");
  const dinnerCandidates = RESTAURANTS.filter((r) => r.type === "夕食" || r.type === "ディナー");
  ...
  const spotEntrance =
    req.style.includes("文化・歴史探訪") ||
    req.likes.includes("水族館・動物園・博物館")
      ? 1500
      : 500;
  const totalBudgetNum =
    Number(lunchBudget) + Number(dinnerBudget) + transportCost + spotEntrance;
